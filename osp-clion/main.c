...
void run_loopback(float *inL, float *inR, float *outL, float *outR, unsigned long len)
{
	float inL32[32];
	float inR32[32];
	...
	resample_32_48(resampleL, outL32, outL, 32);
	resample_32_48(resampleR, outR32, outR, 32);
}
		...
		while (tcp_running) {
			...
			switch (req) {
				case OSP_REQ_UPDATE_VALUES:
					...
					printf("Read %d, sizeof osp_user_data %lu\n", ret, sizeof(osp_user_data));
					printf("Got no op: %d\n", osp_tmp_data->no_op);
					printf("AFC: %d\n", osp_tmp_data->afc);
					printf("Rear Mics: %d\n", osp_tmp_data->rear_mics);
...
#else
static int stream_alsa(osp_user_data *osp_data, unsigned int samp_rate, unsigned int frames_per_buffer)
{
...
static int run_pa_key(osp_user_data *osp_data, unsigned int samp_rate, unsigned int frames_per_buffer, float attenuation_factor)
{
    ...
    if (stream_alsa(osp_data, samp_rate, frames_per_buffer) < 0) {
        printf("Error initializing port audio stuff\n");
        return -1;
    }
		...
		while (tcp_running) {
			...
			switch (req) {
				...
				case OSP_REQ_GET_UNDERRUNS:
					printf("Received underrun request packet\n");
					//fprintf(stderr, "Sending underruns %d\n", pa_data.aux_data.underruns);
					if (osp_tcp_send_underruns(osp_tcp, 0) < 0) {
						printf("Error sending underrun packet to client\n");
						tcp_running = 0;
					}
					break;
