...
file_record::file_record(int freq, std::string file_, float seconds)
{
    rootPath = "/opt/EWS/public/record/";
    file = file_;
    sampling_freq = freq;
    numSamples_before = sampling_freq * 60;
    numSamples_after = sampling_freq * 60;
    buffer.resize(2);
    ...
    numSamples = (int)(float(sampling_freq) * seconds);
...
void file_record::get_params( float& seconds){
    seconds = ((float)numSamples) / float(sampling_freq);
...
void file_record:: write_to_file()
{
    while(1)
    {
        ...
        audioFile.setSampleRate (sampling_freq);
        t=t+1;
        audioFile.save (file_path + std::string("-") + std::to_string(t) + std::string(".wav"), AudioFileFormat::Wave);
        printf("Recorded\n");
    }
