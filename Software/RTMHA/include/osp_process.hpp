...
//            e_n_bf_[channel] = new circular_buffer(max_dwn_buf+BAND_FILT_LEN, 0.0f);
            ...
            for (int band = 0; band < SIX_BANDS; band++) {
                filters[channel][band] = new fir_formii(subband_filter[band], BAND_FILT_LEN);
            ...
            global_mpo_attack = global_mpo_attack / SIX_BANDS;
            global_mpo_release = global_mpo_release / SIX_BANDS;
...
//        std::cout << std::chrono::duration_cast<std::chrono::microseconds>(elapsed).count() << std::endl;
    ...
    void process_channels(int channel){
        float *sub_data[SIX_BANDS];
        for(int i = 0; i < SIX_BANDS; i++){
...
#endif
            for(int i = 0; i < SIX_BANDS; i++)
                filters[channel][i]->process(e_n_data[channel], sub_data[i], out_size_);
#ifdef STATS
            stats_end_time[channel] = std::chrono::steady_clock::now();
            stats_dur[channel] = std::chrono::duration_cast<std::chrono::duration<double>>(stats_end_time[channel] - stats_start_time[channel]).count() * 1000000;
...
private:
    ...
    fir_formii *filters[DEFAULTS::NUM_CHANNEL][SIX_BANDS];
    noise_management *noiseMangement[DEFAULTS::NUM_CHANNEL][SIX_BANDS];
    peak_detect *peakDetect[DEFAULTS::NUM_CHANNEL][SIX_BANDS];
    wdrc *wdrcs[DEFAULTS::NUM_CHANNEL][SIX_BANDS];
    size_t max_dwn_buf;
...
//    circular_buffer* e_n_bf_[DEFAULTS::NUM_CHANNEL];
    ...
    freping* fp_[DEFAULTS::NUM_CHANNEL][SIX_BANDS];
    float* e_n_data[DEFAULTS::NUM_CHANNEL];
