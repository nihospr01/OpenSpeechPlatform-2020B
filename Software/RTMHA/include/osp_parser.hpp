    ...
    explicit osp_parser(){
        ...
        options
                ->add_options("Control Signals")
                        ...
                        ("file_test", "Test OSP using file playback only 0 - Use Live Input 1 - Use only files", cxxopts::value<int>()->default_value("0"))
                        ("input_channels", "Number of input channels", cxxopts::value<int>()->default_value("2"))
                        ("output_channels", "Number of output channels", cxxopts::value<int>()->default_value("2"))
                        ("ref_design", "0 - 6 Band Implementation; 1 - 10 Band Implementation", cxxopts::value<int>()->default_value("0"))
    ...
    void parse(int argc, char* argv[], osp_user_data *user_data, controls *main_controls, int initial = 0){
        ...
        try {
            ...
            if (result.count("freping_alpha")) {
                ...
                if (freping_alpha_arr.size() == SIX_BANDS) {
                    for (int j = 0; j < SIX_BANDS; j++) {
                        if (channel == 0 || channel == 1)
                    ...
                    std::cout << "For freping_alpha please have " << SIX_BANDS << " bands worth of data." << std::endl;
                }
            }
            ...
            if (result.count("g50")) {
                ...
                if (g50_arr.size() == SIX_BANDS) {
                    ...
                    std::cout << "For g50 please have " << SIX_BANDS << " bands worth of data." << std::endl;
                }
            }
            if (result.count("g80")) {
                ...
                if (g80_arr.size() == SIX_BANDS) {
                    ...
                    std::cout << "For g80 please have " << SIX_BANDS << " bands worth of data." << std::endl;
                }
            }
            if (result.count("knee_low")) {
                ...
                std::vector<std::string> knee_low_arr((std::istream_iterator<WordDelimitedBy<','>>(iss)),
                                                      std::istream_iterator<WordDelimitedBy<','>>());
                if (knee_low_arr.size() == SIX_BANDS) {
                    for (int j = 0; j < SIX_BANDS; j++) {
                        if (channel == 0 || channel == 1)
                    ...
                    std::cout << "For knee_low please have " << SIX_BANDS << " bands worth of data." << std::endl;
                }
            }
            if (result.count("mpo_band")) {
                ...
                std::vector<std::string> knee_high_arr((std::istream_iterator<WordDelimitedBy<','>>(iss)),
                                                       std::istream_iterator<WordDelimitedBy<','>>());
                if (knee_high_arr.size() == SIX_BANDS) {
                    for (int j = 0; j < SIX_BANDS; j++) {
                        if (channel == 0 || channel == 1)
                    ...
                    std::cout << "For mpo_band please have " << SIX_BANDS << " bands worth of data." << std::endl;
                }
            }
            if (result.count("attack")) {
                ...
                if (attack_arr.size() == SIX_BANDS) {
                    ...
                    std::cout << "For attack please have " << SIX_BANDS << " bands worth of data." << std::endl;
                }
            }
            if (result.count("release")) {
                ...
                if (release_arr.size() == SIX_BANDS) {
                    ...
                    std::cout << "For release please have " << SIX_BANDS << " bands worth of data." << std::endl;
                }
            }
...
//                    std::cout<<result["audio_filename"].as<std::string>()<<std::endl;
            ...
            if(result.count("input_channels")){
                if(initial == 0){
                    std::cout << "Can't change Input device after startup" << std::endl;
                }
                else {
                    main_controls->input_channels = result["input_channels"].as<int>();
                    if(main_controls->input_channels > 2){
                        std::cout << "Reducing to 2 channels" << std::endl;
                        main_controls->input_channels = 2;
                    }
                    else if(main_controls->input_channels < 0){
                        std::cout << "Please don't enter negative numbers" << std::endl;
            ...
            if(result.count("output_channels")){
                if(initial == 0){
                    std::cout << "Can't change Input device after startup" << std::endl;
                }
                else {
                    main_controls->output_channels = result["output_channels"].as<int>();
                    if(main_controls->output_channels > 2){
                        std::cout << "Reducing to 2 channels" << std::endl;
                        main_controls->output_channels = 2;
                    }
                    else if(main_controls->output_channels < 0){
                        std::cout << "Please don't enter negative numbers" << std::endl;
                        main_controls->output_channels = 0;
                    }
                }
            }
            ...
            if(result.count("file_test")){
                if(initial == 0){
                    std::cout << "Can't change multithreading after startup" << std::endl;
                }
                else {
                    main_controls->file_test = (bool) result["file_test"].as<int>();
                }
            }
            if(result.count("ref_design")){
                if(initial == 0){
                    std::cout << "Can't change multithreading after startup" << std::endl;
                }
                else {
                    main_controls->ref_design = result["ref_design"].as<int>();
                }
            }
