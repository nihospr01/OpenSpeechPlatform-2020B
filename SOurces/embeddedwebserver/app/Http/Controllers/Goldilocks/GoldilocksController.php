...
class GoldilocksController extends Controller
{
    ...
    public function researcherPage(){
        ...
        if(session()->get('researcher', 'NULL') != 'NULL' && session()->get('listener', 'NULL') != 'NULL'){
            ...
            foreach($programs as $p){
                $parameters[$p->id] = [
                    'parameters' => json_decode($p->parameters),
                    'parametersTwoChannels'=> json_decode($p->parametersTwoChannels)
                ];
            }
    ...
    public function selfAdjustment($program_id = null){
        ...
        $data = [
            'parameters' => null,
            'parametersTwoChannels' => null
        ];
        $parameters = null;
        $next_name = "";
        if($program_id){
            $program = GoldilocksProgram::where('id', $program_id)->firstOrFail();
            $data['parameters']  = json_decode($program->parameters);
            $data['parametersTwoChannels'] = json_decode($program->parametersTwoChannels);
        ...
        if($program_id == null && $listener->current_program_id != null) {
            ...
            $app_behavior = $data['parameters']->app_behavior;
