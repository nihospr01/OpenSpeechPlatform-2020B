...
router.post("/getparam", (req, res, next) => {
    ...
    client.on('error', function(err) {
        res.status(500).send("Getting Params Failed. Please make sure MHA is running");
        client.end();
        return;
    });
})
...
// commented out until someone explains it to me.
...
router.post("/confusionMatrix", async(req, res, next) => {
    try {
        var spawn = require('child_process').spawn,
        py = spawn('python', ['confusion_matrix.py']),
